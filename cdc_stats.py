        ...
        if current_time - self.last_stats_time >= self.stats_interval:
            ...
            if len(self.latencies) > 0:
                ...
                if len(recent_times) > 1:
                    # Calculate throughput based on message arrival rate
                    throughput = len(recent_times) / (max(recent_times) - min(recent_times))
                else:
                    throughput = 0

                # Calculate average replication and delivery latencies
                avg_replication_latency = np.mean(np.array(self.replication_latencies)) if self.replication_latencies else None
                avg_delivery_latency = np.mean(np.array(self.delivery_latencies)) if self.delivery_latencies else None
                
                # Prepare stats row with conditional values
                time_elapsed = int((current_time * 1000) - self.start_time)  # Convert to milliseconds
                stats_row = [
                    ...
                    round(p99, 2),
                    round(throughput, 2),
                    round(avg_replication_latency, 2) if avg_replication_latency is not None else '',  # Empty if no data
                    round(avg_delivery_latency, 2) if avg_delivery_latency is not None else ''         # Empty if no data
                ]
                ...
                stats_msg = (f"Stats saved - Avg Latency: {avg_latency:.2f}ms, "
                            f"P50: {p50:.2f}ms, P95: {p95:.2f}ms, P99: {p99:.2f}ms, "
                            f"Throughput: {throughput:.2f} msgs/sec, "
                            f"Consumer Lag: {consumer_lag} messages")
                
                # Only add replication and delivery stats if available
                if avg_replication_latency is not None:
                    stats_msg += f", Avg Replication Latency: {avg_replication_latency:.2f}ms"
                if avg_delivery_latency is not None:
                    stats_msg += f", Avg Delivery Latency: {avg_delivery_latency:.2f}ms"
